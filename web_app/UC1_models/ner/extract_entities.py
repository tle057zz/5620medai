...
def is_false_chemical(text):
    ...
    # Ignore very short chemical names and non-alphabetic terms
    if len(text_clean) < 3 or (len(tokens) <= 2 and text_clean.isalpha()):
        return True

    # Check for common chemical suffixes
    ...
    # Skip matches with capitalized names that are likely people or locations
    if re.match(r"^([A-Z][a-z]+(\s[A-Z][a-z]+){0,2})$", text_clean):
        return True

    # Skip terms that look like professional titles or similar
...
if __name__ == "__main__":
    ...
    # )
    input_path = "/Users/thanhle/Library/CloudStorage/GoogleDrive-lenhothanh.nsl@gmail.com/.shortcut-targets-by-id/1Je2GU6cAmriwQ_9lhORCt8JeHBjH-2Yq/ELEC5620/Code/5620medai/web_app/UC1_models/sectionizer/sectionized_output.json"
