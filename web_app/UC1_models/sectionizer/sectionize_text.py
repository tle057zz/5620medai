...
def sectionize_text(input_path: str, verbose: bool = True) -> dict:
    """
    Reads the OCR text file and splits it into sections based on headings relevant to health forms.
    ...
    # -----------------------------
    # Regex pattern to detect specific health form sections
    # We aim to capture sections related to health conditions, medications, medical history, etc.
    # -----------------------------
    pattern = re.compile(
        r"(CURRENT\s*HEALTH\s*CONDITIONS|MEDICATIONS|PAST\s*MEDICAL\s*HISTORY|FAMILY\s*MEDICAL\s*HISTORY|BLOOD\s*PRESSURE|BMI|LAB\s*VALUES|ALCOHOL\s*CONSUMPTION|SMOKING\s*STATUS|PAST\s*SURGERIES|PAST\s*HOSPITALIZATIONS)",
        re.IGNORECASE,
    )
...
if __name__ == "__main__":
    ...
    input_path ="/Users/thanhle/Library/CloudStorage/GoogleDrive-lenhothanh.nsl@gmail.com/.shortcut-targets-by-id/1Je2GU6cAmriwQ_9lhORCt8JeHBjH-2Yq/ELEC5620/Code/5620medai/web_app/UC1_models/ocr/Medical_History_output.txt"
    try:
