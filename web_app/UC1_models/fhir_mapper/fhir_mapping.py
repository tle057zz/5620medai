...
def make_condition(ent, patient_id, pract_id):
    """Create a FHIR Condition resource based on the entity."""
...
def make_medication(ent, patient_id, pract_id):
    """Create a FHIR MedicationStatement resource based on the entity."""
...
def make_observation(ent, patient_id, pract_id):
    """Create a FHIR Observation resource based on the entity."""
    code = ent.get("linked_code") or "000000"
...
def make_procedure(ent, patient_id, pract_id):
    """Create a FHIR Procedure resource based on the entity."""
    return {
...
def make_encounter(patient_id):
    """Create a minimal Encounter resource."""
    return {
...
def is_valid_observation(text):
    """Check if the observation text is valid."""
    t = text.lower().strip()
    if len(t.split()) < 2:
        return False
...
def convert_to_fhir_bundle(linked_json_path, output_path):
    """Convert linked entities to FHIR bundle."""
    ...
    # Safe Name Extraction from Linked Entities
    for section_name, ents in data.items():
        if not isinstance(ents, list):
            continue  # skip non-list sections
        for e in ents:
    ...
    # Adding Conditions, Medications, Observations, etc.
